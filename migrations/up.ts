import { MongoClient } from 'mongodb'
import { config } from 'dotenv'
import path from 'path'

// Load env from .env.local then fallback to .env
config({ path: path.resolve(process.cwd(), '.env.local') })
config()

async function run() {
  const uri = process.env.MONGODB_URI
  if (!uri) throw new Error('MONGODB_URI is not set')
  const client = new MongoClient(uri)
  await client.connect()
  const url = new URL(uri)
  const dbName = (url.pathname && url.pathname !== '/') ? decodeURIComponent(url.pathname.slice(1)) : undefined
  const db = client.db(dbName)

  // Create collections and indexes
  const conversations = db.collection('conversations')
  const messages = db.collection('messages')

  await conversations.createIndex({ userId: 1, updatedAt: -1 })
  await messages.createIndex({ conversationId: 1, createdAt: 1 })

  // Optional: backfill titles for untitled conversations (first user message line)
  const cursor = conversations.find({ $or: [{ title: null }, { title: '' }] })
  while (await cursor.hasNext()) {
    const conv = await cursor.next()
    if (!conv) break
    const firstUser = await messages.find({ conversationId: conv._id, role: 'user' }).sort({ createdAt: 1 }).limit(1).next()
    if (firstUser?.content) {
      const title = firstUser.content.split('\n')[0].slice(0, 60)
      await conversations.updateOne({ _id: conv._id }, { $set: { title } })
    }
  }

  await client.close()
  console.log('Migration up complete')
}

run().catch((err) => {
  console.error(err)
  process.exit(1)
})

